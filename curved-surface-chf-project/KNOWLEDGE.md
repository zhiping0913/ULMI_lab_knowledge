...
- `a0_50_reflection_gain_3d_estimate_2026-07-19.md` — a0=50, ND/a0=0.30, reflection focus-at-y workflow: generated `focus.tsv`, propagated six `reflection_focus.nc` fields, sliced y=0, ran `spectrum_1D.py`, computed harmonic gains/phase statistics, and produced conservative/ideal estimated 3D `E_max(K)` plots.
...
See supporting note `a0_50_reflection_gain_3d_estimate_2026-07-19.md` for the follow-up focus-at-y and estimated 3D workflow requested on 2026-07-19: generated `a0=50/2D/focus.tsv`, propagated six `reflection_focus.nc` fields with `propagate_2D_at_time.py` (`300G/1h`, MaxRSS ~244 GB), sliced `y=0`, reran `spectrum_1D.py` with correct `LASER_A0=50`, computed per-harmonic gains and phase statistics, and plotted conservative estimated 3D `E_max(K)`. Result for `ND/a0=0.30`: conservative `E3D≈588 Ec` at `K=-0.012`, `≈533 Ec` at `K=-0.010`, with ideal upper `≈0.9–1.1e3 Ec`; output dir `utility/plot/a0_50_reflection_gain_3d_estimate_20260719/`.
...
## Coherent focusing / unfiltered attosecond-pulse mechanism (2026-07-19)

See supporting note `coherent_focusing_unfiltered_pulse_literature_2026-07-19.md` for the mechanism/literature answer to Zhiping's question about why coherent harmonic focusing can produce an attosecond-scale local raw `Bz` pulse without explicit spectral filtering. Key point: at the harmonic focus, curved-surface propagation provides a frequency-dependent spatial transfer function that preferentially weights phase-locked high harmonics; Gordienko et al. PRL 2005 explicitly describe CHF as working as a spectral filter, while Vincenti/Quéré CRM papers provide the modern curved-plasma-mirror implementation and Schwinger-limit context.

See supporting note `local_harmonic_spectrum_diagnostic_2026-07-19.md` for Zhiping's requested first-pass `n=1..30` local raw-`Bz` harmonic diagnostic (`G_n` versus 1D reference and wrapped `φ_n-nφ_1` residual) using an independent local Fourier projection. It showed strong order-dependent gain but jagged wrapped residuals, and was superseded by the spectrum_1D.py-based analysis below.

See supporting note `spectrum1d_gain_phase_3d_estimate_2026-07-19.md` for the corrected canonical analysis using `spectrum_1D.py` per-harmonic TSV normalization/phase. Result: for `ND/a0=0.30`, `side=reflection`, 2D focus centerline gains over 1D are strongest for `K=-0.005/-0.008`, with `G_2D,E≈4–6` over much of `n≈5–20` and maxima near `n=15` of `≈10–11`; ideal symmetric 3D field gain estimate is `G_3D,E≈G_2D,E²` (up to `~100` field gain in the strongest band, before Strehl/aperture/coherence penalties). Direct `phase_at_peak` from spectrum_1D.py clusters tightly for 2D cases over `n=5–15` (std `~0.02–0.03π`, versus 1D `~0.15π`), while `n≈20–30` wraps/scatters.

See supporting note `estimated_3d_Emax_vs_K_2026-07-19.md` for the requested estimated 3D focus `E_max(K)` plot based on existing 2D `focus.tsv` and locked-band (`n=1..20`) spectrum_1D.py harmonic gains. Conservative model uses `E3D_conservative = E2D * sqrt(G_locked)`, where `G_locked` is an amplitude-weighted 2D/1D field gain over the locked band; ideal upper uses `E2D * G_locked`. For `ND/a0=0.30`, best conservative estimates are `K=-0.005: E3D≈330 Ec` and `K=-0.008: E3D≈321 Ec`, with ideal upper `≈670–695 Ec`.
